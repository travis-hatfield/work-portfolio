...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col gap-14">
      <section className="bg-noise-fade -mx-6 -mt-12 rounded-b-3xl px-6 pb-12 pt-16 sm:-mx-8 sm:px-8">
        ...
        <p className="mt-3 max-w-xl text-lg text-muted">{profile.title}</p>
        <p className="mt-6 max-w-2xl leading-relaxed text-foreground/90">{profile.blurb}</p>
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          <a
            ...
            className="rounded-lg border border-border bg-card px-5 py-2.5 text-sm font-medium transition-colors hover:border-accent"
          >
            {profile.email}
          </a>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        {professionalLinks.map((l) => (
          <Link
            key={l.href}
            href={l.href}
            className="group rounded-xl border border-border bg-card p-6 transition-all hover:-translate-y-0.5 hover:border-accent hover:shadow-md"
          >
